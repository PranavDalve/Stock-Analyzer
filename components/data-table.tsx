...
export function DataTable({ data: _ignored }: { data: unknown[] }) {
  ...
  // Only use real data — no placeholder rows
  const tableData: BalanceSheetRow[] = stockData?.balanceSheet ?? []
  ...
  // Compute QoQ revenue growth for badge
    ...
    if (prev.revenue > 0) {
      ...
      revGrowthBadge = (
        <Badge variant="outline">
          {growth >= 0 ? <TrendingUpIcon className="h-3 w-3" /> : <TrendingDownIcon className="h-3 w-3" />}
    ...
    <div className="px-4 lg:px-6">
      <Card>
        <CardHeader className="flex flex-row items-start justify-between">
          <div>
            <CardTitle>
              {stockData ? `${stockData.symbol} — Quarterly Financials` : "Quarterly Financials"}
            </CardTitle>
            <CardDescription>
              {stockData
                ? "Revenue, Net Income, EPS & Balance Sheet — Last 4 Quarters"
                : "Search a stock above to view quarterly financials"}
            </CardDescription>
          </div>
          {revGrowthBadge}
        </CardHeader>
        ...
        <CardContent>
          ...
          {/* Empty state — no stock searched yet */}
          {!loading && !stockData && (
            <div className="h-40 flex flex-col items-center justify-center gap-2 text-muted-foreground">
              <SearchIcon className="h-8 w-8 opacity-30" />
              <p className="text-sm">Search for a stock to see quarterly financials</p>
            </div>
          )}
                    ...
                    {table.getRowModel().rows.length ? (
                      ...
                      <TableRow>
                        <TableCell colSpan={columns.length} className="h-24 text-center text-muted-foreground">
                          No quarterly data available for this stock
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
